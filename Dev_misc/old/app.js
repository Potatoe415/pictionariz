...
(() => {
  ...
  function render(){
    ...
    // Use session order to select the active card (no repeats)
    const idx = sessionState?.order?.[sessionState.pos] ?? 0;
    const card = CARDS[idx];

    // IMPORTANT: use WORD COLOR for the block
    const cardHex = resolveColor(card.card_color);
    const wordHex = resolveColor(card.word_color);

    // Dot (category) uses card_color
    ...
    if (hintText){
      ...
      // REQUIRED: remove hint div if empty -> best we can do here is hide it
      // (We keep the element to avoid DOM churn, but it's not displayed.)
      elHint.textContent = "";
      elHint.style.display = "none";
    }
  ...
  function resetToSplash(){
    ...
    // Clear session order so you can replay without repeats in the new session
    ...
    if (pictionaryBadge) pictionaryBadge.textContent = "Tap to start";
    if (splashError) splashError.classList.remove("show");
...
async function startPictionary(){
  ...
  // ===== Events =====
  if (topImageEl) topImageEl.addEventListener("click", (e) => {
    e.preventDefault();
    resetToSplash();
  });
  ...
  if (topImage) topImage.addEventListener("click", resetToSplash);
